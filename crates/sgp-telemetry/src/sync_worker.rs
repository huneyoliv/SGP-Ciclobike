...
use crate::export::strava::StravaClient;
...
impl SyncWorker {
    ...
    pub fn new(
        ...
        let strava = if sgp_core::strava_app_configured() {
            sgp_core::StravaTokens::load().map(StravaClient::new)
        } else {
            None
        };
        ...
        loop {
            ...
            match self.buffer.drain_batch(50) {
                Ok(points) => {
                    ...
                    tracing::info!(
                        "Drenados {} pontos do disco. Tentando envio...",
                        points.len()
                    );
                    ...
                    if let Some(ref host) = self.mqtt_host {
                        if self
                            .send_via_mqtt(host, self.mqtt_port, &points)
                            .await
                            .is_ok()
                        {
                            success = true;
                    ...
                    // 3. Canal Terciário (Fallback): Strava GPX
                    if !success {
                        if let Some(ref mut strava) = self.strava {
                            let gpx_content = crate::export::gpx::GpxEncoder::encode(
                                &points,
                                "Pedalada SGP-Ciclobike",
                            );
                            if strava
                                .upload_gpx(&gpx_content, "Pedalada SGP-Ciclobike")
                                .await
                                .is_ok()
                            {
                                success = true;
                    ...
                    if success {
                        tracing::info!(
                            "Lote de telemetria sincronizado e removido do disco com sucesso!"
                        );
                    } else {
                        tracing::warn!(
                            "Falha de conexão com os servidores. Devolvendo lote ao disco."
                        );
    ...
    async fn send_via_mqtt(
        &self,
        host: &str,
        port: u16,
        points: &[crate::snapshot::TrackPoint],
    ) -> Result<(), String> {
        ...
        tokio::spawn(async move { while let Ok(_notification) = eventloop.poll().await {} });
        ...
        client
            .publish("sgp/telemetry", QoS::AtLeastOnce, false, payload.as_bytes())
            .await
...
mod tests {
    ...
    #[test]
    fn test_sync_worker_new() {
        let path = std::path::Path::new("dummy_sync_worker.db");
        let worker = SyncWorker::new(&path, Some("localhost"), 1883, Some("http://localhost"));
        assert_eq!(worker.mqtt_host, Some("localhost".to_string()));
        assert_eq!(worker.mqtt_port, 1883);
        assert_eq!(worker.http_endpoint, Some("http://localhost".to_string()));
    }
}
